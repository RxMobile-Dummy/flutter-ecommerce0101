...
class AdminServicesRepositoryImpl extends AdminServicesRepository{
  ...
  @override
  Future<Either<Failure, String>> sellProduct(String token, String name, String description, double price, double quantity, String category, List<File> images)
   async => await adminServicesDataSource.sellProduct(token, name, description, price, quantity, category, images);

  @override
  Future<Either<Failure, List<ProductEntity>>> fetchAllProducts(String token)
  async => await adminServicesDataSource.fetchAllProducts(token);

  @override
  Future<Either<Failure, String>> deleteProduct(String token, ProductEntity productEntity)
    async => await adminServicesDataSource.deleteProduct(token,productEntity as ProductModel);

  @override
  Future<Either<Failure, List<OrderEntity>>> fetchAllOrders(String token)
    async => await adminServicesDataSource.fetchAllOrders(token);
